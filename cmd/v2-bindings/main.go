...
func run(args []string, stdout, stderr io.Writer) error {
	...
	for _, field := range []struct {
		...
		{name: "--public-key", value: opts.publicKeyPath},
		{name: "--catalog-service-digest", value: opts.catalogServiceDigest},
		{name: "--catalog-extractor-digest", value: opts.catalogExtractorDigest},
		{name: "--output", value: opts.output},
	} {
		if field.value == "" {
	...
	if err != nil {
		return fmt.Errorf("read RSA public-key PEM %q: %w", opts.publicKeyPath, err)
	}
	bindings, keyPolicy, err := release.GenerateV2Bindings(release.V2BindingsInput{
		KeyID:                  opts.keyID,
		PublicKeyPEM:           publicKeyPEM,
		CatalogServiceDigest:   opts.catalogServiceDigest,
		CatalogExtractorDigest: opts.catalogExtractorDigest,
	})
	if err != nil {
		return err
	}
