...
A release is one immutable component tuple. It holds the repository-owned
frontend, runtime-base, bottle-fetcher, catalog-extractor, and materializer
images together with the external inputs those images were built and verified
against: the upstream Dalec dispatcher, Homebrew, the verification-key set, the
Go modules, the builder images, the Ubuntu snapshot, Chisel, and policy. Every
member of the tuple is pinned by digest or exact version, then built, tested,
and signed once. Promotion adds references to already tested owned-component
digests; it never rebuilds or re-resolves them.

Each component index is published under the release version tag, for example
`ghcr.io/sozercan/dalec-homebrew:v0.2.9`. Platform children never receive a
version tag: read a child digest from that published index or from the
release's signed `components.json`.

## Who this page is for

This page is for maintainers and release operators, the people who cut a
release, promote it, or roll one back. To build an image from an already
published release, read [`usage.md`](usage.md) instead. For the trust
boundaries these rules protect, read [`../SECURITY.md`](../SECURITY.md).
...
Formula `share/doc` content remains retained. A policy change requires a newly
built, tested, and signed component tuple.

The `dalec-homebrew-prune-manifest/v4` schema and its reason vocabulary remain
compatible with the inactive `transitive_runtime_share_doc` reason, but a new
release must not emit that reason. Release integration requires all six active
reasons and rejects an inactive share/doc record in either explicit entries or
subtree commitments.

### Build options that fill compiled bindings
...
parent index claim. Both identities must be digest-pinned; mutable tags and
cross-repository index/child pairs are rejected. Release signing independently
proves that the claimed index contains the executing platform child before
accepting the evidence. Each resolution record binds both identities, the
selected runtime-base and materializer children, and the immutable
bottle-fetcher and catalog-extractor indexes compiled into the frontend.

### Component manifest

`components.json` records the complete tuple. Current manifests use
`dalec-homebrew-components/v2` and carry:

- multi-platform `frontend`, `runtime_base`, `materializer`, `bottle_fetcher`,
  and `catalog_extractor` components;
- the Homebrew commit, portable Ruby version, verification-key-set digest, and
  metadata-bundle digest;
- `tap_policy_digest` and `executable_runtime_policy_digest`;
- the exact supported catalog, fetch, and provenance policy-version sets,
  including the Sigstore, receiptless HTTPS source-waiver, build-local artifact,
  and prebuilt-derived-bottle provenance identifiers;
- the pinned Dalec and BuildKit Go module versions.

See [`../release/components-v2.example.json`](../release/components-v2.example.json)
for that shape; the checked-in file contains placeholders and is illustrative.
[`../release/components.example.json`](../release/components.example.json) shows
the older `dalec-homebrew-components/v1` shape. The decoder still accepts those
archived records, but release automation generates the current shape with
...
`cmd/v2-bindings --catalog-extractor-ref` emits the canonical build-local policy
bindings that the materializer and frontend compile in. No RSA catalog key, key
rotation, catalog-service deployment, persistent writer, or public HTTPS origin
is required.
...
4. platform materializer children bound to the helper indexes and
   corresponding full Ubuntu children, then the materializer index
...
The materializer and frontend compile in the exact bottle-fetcher and
catalog-extractor references plus the policy digests and supported-version sets.
[`../docker-bake.hcl`](../docker-bake.hcl) exposes the repository build
targets. Signing, provenance, vulnerability scanning, OCI referrer publication,
mirror retention, and promotion are release-pipeline responsibilities and are
deliberately not performed by the frontend.

## Metadata bundle and its freshness window

Each release captures and embeds exactly one authenticated Homebrew metadata
bundle and compiles that bundle's manifest digest into both frontend children.
Every build that uses the release must supply the same bundle.

Release-bound frontends reject metadata older than 168h (7 days), so a published
release only builds within seven days of the capture that produced it. After
that window a solve fails with:

```text
authenticate Homebrew metadata bundle: load metadata bundle snapshot: formula.jws.json freshness: metadata is stale: generated <ts>, now <ts>, maximum age 168h0m0s
```

`DALEC_HOMEBREW_METADATA_MAX_AGE` can shorten that window but never extend it. A
release-bound frontend rejects a larger value with `release-bound metadata max
age 336h0m0s exceeds 168h`. Cut a new release instead of trying to widen the
window.

Every release integration receives the same release-captured Formula and
migration JWS envelopes. Their bundle digest is compiled into both frontend
children and must also arrive as the top-level
`DALEC_HOMEBREW_METADATA_BUNDLE_DIGEST` build argument with the fixed
`dalec-homebrew-metadata` context. Reconciliation still requires the same
authenticated Homebrew commit, signer, payload digests, and rollback identity
across every spec/platform observation and retains both envelope digests for
each one. `metadata-snapshot.json` deterministically selects the earliest
observation and binds its exact envelope pair. Another capture of the same PS512
payload can have different randomized signature bytes, but one release never
mixes captures. The workflow does not compare the accepted snapshot with earlier
...
- `chisel-releases` commit `f42d76490045602d83de8afef5126987179a6693` and its
  archive SHA-256
...
the external Dalec dispatcher binding under schema
`dalec-homebrew-release-inputs/v2`. The exact references and hashes live beside
the build instructions in [`../Dockerfile`](../Dockerfile),
...
digest-pinned image. They also pin binfmt, Syft, Trivy, Cosign, and every
external GitHub Action; `provenance.json` records the Buildx, BuildKit, binfmt,
Syft, and Trivy identities used for the release. It also records the external
Dalec frontend index as an OCI material and records the complete dispatcher
binding as an invocation parameter.
...
`dalec-homebrew`. The child advertises `image` only for upstream route
discovery. Direct use of `dalec-homebrew` as the syntax frontend is outside the
release contract and is rejected when the forwarded `homebrew` target context is
absent.
...
Release-verified builds always use the digest-pinned references in that binding,
never a mutable tag such as `ghcr.io/project-dalec/dalec/frontend:latest`.

`scripts/release-inputs.sh` strictly decodes the file, validates every OCI
...
accept the bundle, because recovery never rebuilds the tuple.

### Retry or resume a run

For an explicit retry or draft recovery, dispatch the trusted workflow on
`main`:
...
rebuilds the tuple and may leave the unique `build-<sha>-<run>-<attempt>`
staging tags from the failed run. Once the exact signed draft exists, rerunning
the dispatch can resume after a partial promotion; matching component version
tags are accepted and only missing tags are created. The workflow does not clean
up staging tags. Registry retention and cleanup must preserve the digest
subjects and OCI referrers needed to verify or resume the release.

### What a new-tuple run does
...
3. Runs every focused integration spec through the pinned platform-specific
   ...
   reports for each component child. Fixed `CRITICAL` findings are surfaced in
   the corresponding evidence job summary and retained in the reports without
...
4. Generates provenance that includes the external dispatcher binding, signs
   ...
   manifest, metadata snapshot, and checksum set. The upstream Dalec image is
   neither signed nor promoted by this repository.

### Access and environment

The GitHub `release` environment gates signing and promotion. Configure required
reviewers, protect release-critical paths with branch rules, restrict
release-tag creation to release operators, and use tag rules to prevent update
or deletion. GHCR and GitHub Release writes use the scoped `GITHUB_TOKEN`;
keyless Cosign uses GitHub Actions OIDC. Push release tags individually.

## End-to-end release requirements
...
1. Build `amd64` and `arm64` runtime-base children from the pinned Chisel
   binary, immutable `chisel-releases` commit, and Ubuntu snapshot with
   `SOURCE_DATE_EPOCH` fixed, then assemble their index.
2. Build the bottle-fetcher and catalog-extractor children and indexes, then
   build materializer children from the corresponding pinned full Ubuntu child
   images with the exact helper and policy tuple compiled in.
...
4. Capture and verify one Homebrew metadata bundle, bind its digest into both
   ...
   reports. The core matrix must exercise automatic runtime minimization,
   including requested-Formula preservation, all six active safe pruning
   classes, retained Formula `share/doc` and runtime-sensitive paths, rejection
   of inactive share/doc prune records, and a size report for the minimized
   runtime.
5. Sign all ten component children and five indexes, attach the SLSA v0.2
   ...
   checksum set.
6. Resolve once, retain the signed metadata envelopes and resolution records,
   and mirror every selected layer by digest.
7. Build platform runtime images, test and scan them by manifest digest, then
   assemble the final index from those exact manifests.
8. Attach signed SPDX, provenance, resolution, inventory, prune,
   materialization, base evidence, and vulnerability or VEX evidence.
9. Promote by adding references to existing digests; never rebuild or
   re-resolve during promotion.

### What release CI must reject
...
### Vulnerability scans are evidence, not gates

Component vulnerability scans are evidence-producing checks, not release gates.
Trivy scans every component child for fixed `CRITICAL` findings and must
...
- one `runtime-minimization-<platform>.json` size report per platform
...
Promotion compares the complete decoded SLSA predicate after treating only the
four boolean false-or-omitted fields that Cosign v3 normalizes
(`metadata.completeness.parameters`, `metadata.completeness.environment`,
`metadata.completeness.materials`, and `metadata.reproducible`) as equivalent;
every other value and field remains exact. SPDX predicates remain exact.
...
Build-local tap ingestion records the exact Git commit, tree/archive digests,
canonical catalog digest, extractor reference, and explicit
`build-local-exact-commit-no-cross-build-rollback-v1` evidence. It does not
provide centralized cross-build rollback floors: a later build can observe a
force-pushed default branch. Promotion and rollback therefore retain catalog
documents, source archives, derived or upstream bottle bytes, resolution
records, and final image digests; they never reconstruct an old release from a
mutable tap.
