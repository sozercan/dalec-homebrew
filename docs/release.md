...
1. Build amd64/arm64 runtime-base children from the pinned Ubuntu child digests.
2. Build materializer children from the corresponding runtime-base children.
...
Release CI must reject `DALEC_SKIP_TESTS`, mutable component references, conflicting `SOURCE_DATE_EPOCH`, unsigned/mixed component tuples, and exporter settings that differ between test and promotion.
