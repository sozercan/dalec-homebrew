...
For an authorized Formula, build-local ingestion:
...
5. Records the upstream archive, verification, derivation recipe, and derived-bottle identities separately in build-local extraction evidence.

The frontend carries the exact derived bottle bytes directly as a content-addressed BuildKit state, and the materializer uses the existing network-disabled bottle installation and containment path. The Formula's `install` and `post_install` methods are not executed for derived bottles.
...
Rejected. Evidence must distinguish the original upstream archive from the build-locally derived bottle and bind both digests.
